...
#include "SkipList.h"
#include "libatomic/atomic_ops.h"
#include <assert.h>
#include <getopt.h>
#include <limits.h>
#include <pthread.h>
...
void print_skiplist(struct SkipList *set) {
  struct Node *curr;
  int i, j;
  int arr[levelmax];
...
void *test(void *data) {
  ...
  val_t val = 0;
  int unext;
  struct Node **preds = (struct Node **)malloc(levelmax * sizeof(struct Node *));
  struct Node **succs = (struct Node **)malloc(levelmax * sizeof(struct Node *));
...
void *test2(void *data)
{
  ...
  int main(int argc, char **argv)
  {
    ...
    set = constructSkipList(levelmax);
    stop = 0;
